...
class SearchBySFEM_ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    /// tableView for SFEM
    @IBOutlet weak var SFEM_TableView: UITableView!
    
    /// UI CONNECTION SETUP - buttons
    /// when the add sfem button is pressed, perform segue to add sfem page tab
    @IBAction func AddFEMbuttonPressed(_ sender: Any) {
        performSegue(withIdentifier: "AddSFEM_PageSegue", sender: nil)
    }
    ...
    override func viewDidLoad() {
        ...
        reloadTableViews([buildingTableView, floorTableView, SFEM_TableView])
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case floorTableView:
            ...
            Buildings.share.currentArea = 0
            reloadTableViews([floorTableView, SFEM_TableView])
        default:
            Buildings.share.currentArea = indexPath.row
            reloadTableViews([SFEM_TableView])
            performSegue(withIdentifier: "EditSFEM_PageSegue", sender: nil)
        }
