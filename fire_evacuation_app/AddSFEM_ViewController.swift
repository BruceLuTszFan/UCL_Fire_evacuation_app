...
class AddSFEM_ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    /// when the done button is pressed, run validation for input, if correct create a new object of SFEM and add it to the target floor specified by user, if incorrect activate empty field alert
